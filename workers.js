...
async function handleCreatePayPalOrder(
      ...
      return json(
        {
          success: false,
          error:
            "beatId and licenseKey are required"
        },
        400,
        corsHeaders
      );
    ...
    return json(
      {
        success: false,
        error:
          "Cart is empty or invalid items"
      },
      400,
      corsHeaders
    );
  ...
  // ------------------------------------------------------
  // RESOLVE PRODUCTS FROM FIRESTORE
  ...
  for (
        ...
        return json(
          {
            success: false,
            error:
              `Sound kit not found: ${item.itemId}`
          },
          404,
          corsHeaders
        );
        ...
        return json(
          {
            success: false,
            error:
              `Invalid sound-kit price: ${item.itemId}`
          },
          400,
          corsHeaders
        );
      ...
      return json(
        {
          success: false,
          error:
            `Invalid price for ${item.itemId} (${item.licenseKey})`
        },
        400,
        corsHeaders
      );
    ...
    // ----------------------------------------------------
    // STEP 1:
    // Base license price only.
    //
    // Marketing campaign migration comes separately after
    // PayPal create/capture is confirmed working.
    // ----------------------------------------------------
  ...
  // ------------------------------------------------------
  // SAVE paypalOrders/{cartId}
  // ------------------------------------------------------
  //
  // IMPORTANT:
  // The old Firebase system uses the cartId as the
  // Firestore document ID.
  //
  // We therefore use the existing Worker's
  // setFirestoreDocument() helper rather than
  // createFirestoreDocument(), which creates a random ID.
  //
  // ------------------------------------------------------
  ...
  const payload = {
    ...
    purchase_units: [

      {
        ...
        description:
          `Audiory Cart (${resolved.length} item${
            resolved.length > 1
              ? "s"
              : ""
          })`
    ...
    application_context: {
      ...
      return_url:
        `${checkoutBase}/success.html`,

      cancel_url:
        `${checkoutBase}/cancel.html`
    }
  };
  ...
  const response =
    await fetch(
      `${paypalBaseUrl(env)}/v2/checkout/orders`,
      {
        method:
          "POST",
